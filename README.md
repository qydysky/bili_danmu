...
当所选类型在当前直播中不可用时，会按以下顺序[`flv`,`fmp4`]尝试。
...
* 当对配置文件发生**不兼容**的改动(删减原来有效、修改有效项的含义)时，将会发布v0.{y+1}.0版。更新前，你需要关注更新说明，以确认变动的配置项的影响。
* 当对配置文件发生**兼容**的改动(修复、新增功能)时，将会发布v0.{y}.{z+1}版。通常只需要覆盖主体`demo.run`（`demo.exe`）即可更新。
