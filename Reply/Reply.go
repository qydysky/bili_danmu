...
func Reply(common *c.Common, b []byte) {
	reply_log := reply_log.BaseAdd(`返回分派`)
	...
	default:
		reply_log.E("未知的编码方式", head.BodyV)
	}
...
func (t replyF) voice_join_room_count_info(s []byte) {
	...
	msglog.I(`连麦等待人数`, j.Data.ApplyCount)
}
...
func (t replyF) interact_word(s []byte) {
	...
	msglog.BaseAdd("房").LShow(false).I(J.Data.Uname + `关注了直播间`)
}
...
func (t replyF) anchor_lot_start(s []byte) {
	...
	msglog.BaseAdd("房").LShow(false).I(sh...)
}
...
func (t replyF) user_toast_msg(s []byte) {
	...
	{ //额外 ass 私信
		t.Danmu_Main_mq.PushSign(`guard_update`) //使用连续付费的新舰长无法区分，刷新舰长数
		...
		if msg, uid := t.K_v.LoadV(`上舰私信(额外)`).(string), t.K_v.LoadV(`额外私信对象`).(float64); uid != 0 && msg != "" {
			t.Danmu_Main_mq.Push_tag(`pm`, send.Pm_item{
				Uid: int(uid),
				Msg: msg,
			}) //上舰私信-对额外
...
func (t replyF) heartbeat(s int) {
	...
	t.Danmu_Main_mq.Push_tag(`c.Renqi`, struct {
		Roomid int
		Renqi  int
	}{
		Roomid: t.Roomid,
	...
	if renqi_last != s {
		...
		if time.Since(t.Live_Start_Time) > time.Minute {
			watchPerMin = float64(t.Watched) / float64(time.Since(t.Live_Start_Time)/time.Minute)
		}
		if renqi_old != 0 {
		...
		if onlinenum_old != 0 {
			onlinenum_l = (onlinenum_l + 100*float64(t.OnlineNum-onlinenum_old)/float64(onlinenum_old)) / 2
			if onlinenum_l > 0 {
				tmp2 = `+`
		...
		fmt.Printf("+----\n|当前人气:%s%d\n|平均观看:%s%d\n|在线人数:%s%d\n+----\n", tmp, s, tmp1, int(watchPerMin), tmp2, t.OnlineNum)
		renqi_old = s
		watched_old = watchPerMin
		onlinenum_old = t.OnlineNum
	}
	renqi_last = s
	reply_log.BaseAdd(`人气`).LShow(false).I("当前人气", s)
}
...
func (t replyF) win_activity(s []byte) {
	...
	msglog.BaseAdd("房").LShow(false).I("活动", J.Data.Title, "已开启")
}
...
// 	msglog.BaseAdd("礼", "节奏风暴").LShow(false).I(sh...)
// }
...
func (t replyF) room_change(s []byte) {
	...
	// 切换分区
	if t.AreaID != type_item.Data.AreaID {
		t.AreaID = type_item.Data.AreaID
		t.ParentAreaID = type_item.Data.ParentAreaID
	...
	if oldTitle := t.Title; oldTitle != type_item.Data.Title {
		...
		tryS := 900.0
		if v, ok := t.K_v.LoadV("标题修改检测s").(float64); ok && v > tryS {
			tryS = v
		}
...
func (t replyF) room_block_msg(s []byte) {
	...
	msglog.BaseAdd("封").LShow(false).I("用户", J.Uname, "已被封禁")
}
...
func (t replyF) live(s []byte) {
	...
	if err := json.Unmarshal(s, &type_item); err != nil {
		...
		{
			t.Rev = 0.0                    //营收
			t.Liveing = true               //直播i标志
			t.Live_Start_Time = time.Now() //开播h时间
		}
		//开始录制
		go func() {
			if v, ok := t.K_v.LoadV(`仅保存当前直播间流`).(bool); ok && v {
				StreamOStopOther(t.Roomid) //停止其他房间录制
			}
			if _, e := recStartEnd.RecStartCheck.Run(context.Background(), t.Common); e == nil {
				if !StreamOStatus(t.Roomid) {
					StreamOStart(t.Roomid)
				}
			} else {
				msglog.W("房间", type_item.Roomid, e)
			}
			//有时不返回弹幕 开播刷新弹幕
			t.Danmu_Main_mq.PushSign(`flash_room`)
			go replyFunc.RoomSignal.Run2(func(inter replyFunc.RoomSignalI) {
...
func (t replyF) messagebox_user_medal_change(s []byte) {
	...
	if type_item.Data.Medal_name != `` {
		msglog.I("粉丝牌切换至", type_item.Data.Medal_name, type_item.Data.Medal_level)
	}
}
...
func (t replyF) entry_effect(s []byte) {
	...
	msglog.BaseAdd("房").LShow(false).I(copy_writing)
}
...
func (t replyF) roomsilent(s []byte) {
	...
	if J.Data.Level == 0 {
		...
		msglog.I("主播开启了等级禁言:", J.Data.Level)
	}
}
...
func (t replyF) danmu(s []byte) {
	...
	if v, ok := t.K_v.LoadV(`弹幕输出到日志`).(bool); !ok || !v {
		danmulog.LFile("")
		danmulog.LDB(nil, nil, "")
	}
		...
		// 语言tts 私信
		{
			if item.uid != "" {
				...
				if i, e := strconv.Atoi(item.uid); e == nil {
					if msg := t.K_v.LoadV(`弹幕私信`).(string); msg != "" {
						t.Danmu_Main_mq.Push_tag(`pm`, send.Pm_item{
							Uid: i,
							Msg: msg,
						}) //弹幕私信
				...
				if t.K_v.LoadV(`额外私信对象`).(float64) != 0 {
					if msg, uid := t.K_v.LoadV(`弹幕私信(额外)`).(string), t.K_v.LoadV(`额外私信对象`).(float64); uid != 0 && msg != "" {
						t.Danmu_Main_mq.Push_tag(`pm`, send.Pm_item{
							Uid: int(uid),
							Msg: msg,
						}) //弹幕私信-对额外
